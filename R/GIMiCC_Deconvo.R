...
#' 
#' @param 
#' tumor.type Must be one of the following {"IDH" or "GBM"}. IDH should be used 
#' for isocitrate dehydrogenase (IDH) mutant astrocytoma and oligodendroglioma 
#' samples. GBM should be used for IDH wild-type glioblastoma samples
#' 
#' @return 
...
#' 
#' ## Using GIMiCC
#' GIMiCC_results <- GIMiCC_Deconvo(Capper_test_betas, tumor.type = "GBM")
#' head(GIMiCC_results)
#' 
#' @export
...
GIMiCC_Deconvo <- function(Betas, h = 5, tumor.type = "") {
  ...
  for (t in tumor_sample) {
    beta_adj <- c(tumor_beta_iDMC[idmc_dat$hyper == TRUE, t], 
  ...
  proj <- cbind(proj[, -which(colnames(proj) %in% 
  ...
  if (h == "0") {
    ...
    if (h == "1") {
      output <- proj[, c("Tumor", "Immune", "Endothelial and Stromal", 
                         "Glial", "Neuronal")]
    } else {
      if (h == "2") {
        output <- proj[, c("Tumor", "Lymphoid", "Myeloid", "Endothelial", 
                           "Stromal", "Astrocyte", "Microglia", 
                           "Oligodendrocyte", "GABA", "GLU")]
      } else {
        if (h == "3") {
          output <- proj[, c("Tumor", "Tcell", "Bcell", "NK", "Mono", 
                             "Neu", "Endothelial", "Stromal", "Astrocyte", 
                             "Microglia", "Oligodendrocyte", "GABA", "GLU")]
        } else {
          if (h == "4") {
            output <- proj[, c("Tumor", "CD4Tcell", "CD8Tcell", "Bcell", 
                               "NK", "Mono", "Neu", "Endothelial", "Stromal", 
                               "Astrocyte", "Microglia", "Oligodendrocyte", 
                               "GABA", "GLU")]
          } else {
            if (h == "5") {
              output <- proj[, c("Tumor", "CD4nv", "CD4mem", "Treg", "CD8nv", 
